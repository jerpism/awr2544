...
// TODO: these 2 should probably be combined together for one generic input function
// with row/col determined by a variable but that's a problem for future me
void edma_configure_dfft_cols(EDMA_Handle handle, void *cb, void *dst, void *src, uint16_t acnt, uint16_t bcnt, uint16_t ccnt){
    ...
    DebugP_log("EDMA: Configuring doppler cols to hwa\r\n");
    ...
    edmaparam.srcBIdx       = (int16_t) EDMA_PARAM_BIDX(CHIRP_DATASIZE);
    edmaparam.destBIdx      = (int16_t) EDMA_PARAM_BIDX(acnt);
    edmaparam.srcCIdx       = NUM_RX_ANTENNAS * CHIRPS_PER_FRAME * CPLX_SAMPLE_SIZE;
    edmaparam.destCIdx      = CHIRPS_PER_FRAME * CPLX_SAMPLE_SIZE;
    edmaparam.linkAddr      = EDMA_TPCC_OPT(param); 
    edmaparam.srcBIdxExt    = (int8_t) EDMA_PARAM_BIDX_EXT(DOPPLER_COLUMNS * CPLX_SAMPLE_SIZE);
